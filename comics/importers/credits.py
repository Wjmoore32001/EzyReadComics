...
from comics.comicvine.parsing import clean_text, split_comicvine_role_string, to_optional_int
from comics.importers.results import CreditSyncResult
...
def get_or_create_role(role_name):
    ...
    role, created = ComicCreditRole.objects.get_or_create(name=role_name)
...
def sync_issue_person_credits(issue, remote_person_credits, *, dry_run=False):
    ...
    existing_credit_keys = set(
        ComicIssuePersonCredit.objects.filter(issue=issue)
    ...
    with transaction.atomic():
        existing_credits = (
            ComicIssuePersonCredit.objects.select_for_update()
            .filter(issue=issue)
            .select_related("person", "role")
        )
        ...
        for remote_person_credit in remote_person_credits:
            ...
            for role_name in role_names:
                ...
                credit, created = ComicIssuePersonCredit.objects.get_or_create(
                    issue=issue,
                    person=person,
                    role=role,
                    defaults={
...
def sync_volume_person_credits(volume, remote_people, *, dry_run=False):
    ...
    existing_person_ids = set(
        ComicVolumePersonCredit.objects.filter(volume=volume)
    ...
    with transaction.atomic():
        existing_credits = (
            ComicVolumePersonCredit.objects.select_for_update()
        ...
        for remote_person in remote_people:
            ...
            credit, created = ComicVolumePersonCredit.objects.get_or_create(
                volume=volume,
                person=person,
                defaults={
