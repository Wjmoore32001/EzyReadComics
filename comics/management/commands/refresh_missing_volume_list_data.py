...
from comics.comicvine.parsing import clean_text
from comics.importers.volumes import (
...
def get_selected_volumes(*, volume_ids, volume_limit):
    if volume_ids:
        queryset = ComicVolume.objects.filter(comicvine_id__in=volume_ids).order_by("id")
    else:
...
def get_matching_volume_count(*, volume_ids):
    if volume_ids:
        return ComicVolume.objects.filter(comicvine_id__in=volume_ids).count()
