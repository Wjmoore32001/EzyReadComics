...
def print_volume_status(*, command):
    total_volumes = ComicVolume.objects.count()
    refreshed_volumes = ComicVolume.objects.filter(
        list_data_refreshed_at__isnull=False,
    ).count()
    never_refreshed_volumes = ComicVolume.objects.filter(
        list_data_refreshed_at__isnull=True,
    ).count()
    stale_after_refresh_volumes = ComicVolume.objects.filter(
        list_data_refreshed_at__isnull=False,
        date_last_updated__gt=F("list_data_refreshed_at"),
    ).count()
    needs_refresh_volumes = ComicVolume.objects.filter(
        Q(list_data_refreshed_at__isnull=True)
        | Q(date_last_updated__gt=F("list_data_refreshed_at"))
    ).count()

    volumes_with_publisher = ComicVolume.objects.exclude(publisher="").count()
    volumes_with_issue_count = ComicVolume.objects.filter(
        count_of_issues__isnull=False,
    ).count()
    volumes_with_image = ComicVolume.objects.exclude(display_image_url="").count()
    detail_hydrated_volumes = ComicVolume.objects.filter(
        detail_hydrated_at__isnull=False,
    ).count()
    detail_attempted_volumes = ComicVolume.objects.filter(
...
def print_issue_status(*, command):
    total_issues = ComicIssue.objects.count()
    issues_with_comicvine_id = ComicIssue.objects.filter(
        comicvine_id__isnull=False,
    ).count()
    issues_with_volume = ComicIssue.objects.filter(volume__isnull=False).count()
    issues_without_volume = ComicIssue.objects.filter(volume__isnull=True).count()
    issues_with_store_date = ComicIssue.objects.filter(store_date__isnull=False).count()
    issues_with_cover_date = ComicIssue.objects.filter(cover_date__isnull=False).count()
    issues_with_image = ComicIssue.objects.exclude(comicvine_image_original_url="").count()
    detail_hydrated_issues = ComicIssue.objects.filter(
        detail_hydrated_at__isnull=False,
    ).count()
    detail_attempted_issues = ComicIssue.objects.filter(
        detail_hydration_attempted_at__isnull=False,
    ).count()
    associated_images = ComicIssueAssociatedImage.objects.count()
