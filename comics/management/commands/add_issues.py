...
USER_AGENT = "EzyReadComics issue adder"
...
@dataclass
class ImportSummary:
    issue_add_batches_fetched: int = 0
    candidates_checked: int = 0
    issues_created: int = 0
    existing_issues_skipped: int = 0
    missing_data_skipped: int = 0
...
class Command(BaseCommand):
    help = (
        "Add local ComicIssue rows from Comic Vine issue records added by date_added. "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Maximum number of issue add batches to fetch in one run. Defaults to 1.",
        )
    ...
    def handle(self, *args, **options):
        ...
        max_add_batches = options["max_add_batches"]
        dry_run = options["dry_run"]

        validate_options(
            candidate_limit=candidate_limit,
            max_add_batches=max_add_batches,
        )
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("Comic Vine issue adder"))
        ...
        self.stdout.write(f"Maximum issue add batches this run: {max_add_batches}")
...
def validate_options(candidate_limit, max_add_batches):
    ...
    if max_add_batches < 1:
        raise CommandError("max-add-batches must be at least 1.")

    if max_add_batches > 5:
        raise CommandError("This importer only allows max-add-batches up to 5 for now.")
...
def print_import_summary(command, summary):
    command.stdout.write("")
    command.stdout.write(command.style.SUCCESS("Issue add summary:"))
    command.stdout.write(f"Issue add batches fetched: {summary.issue_add_batches_fetched}")
