document.addEventListener("DOMContentLoaded", function () {
  ...
  const statusModalTitle = document.getElementById(
    "tracking-status-modal-title",
  );
  ...
  const statusModalConfirm = document.querySelector(
    "[data-status-modal-confirm]",
  );
  const statusModal = statusModalElement
    ? bootstrap.Modal.getOrCreateInstance(statusModalElement)
    : null;
  ...
  function createTrackingCell(tracking) {
    ...
    if (tracking.tracked) {
      ...
      unfollowOption.textContent =
        tracking.item_type === "volume" ? "Remove" : "Unfollow";
  ...
  function createRunRow(item) {
    ...
    const runTitle =
      item.year && !item.year_muted
        ? `${item.year} — ${item.title}`
        : item.title;

    addCell(row, runTitle, {
      bold: true,
      linkLike: true,
      href: item.row_url,
    });
  ...
  async function postTrackingForm(actionUrl, formData) {
    ...
    if (!response.ok || !data || data.ok === false) {
      const message =
        data && data.error ? data.error : "Could not save tracking status.";
  ...
  document.querySelectorAll("[data-hide-more]").forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      const removableCount = Math.min(
        hideCount,
        Math.max(rows.length - minVisible, 0),
      );
