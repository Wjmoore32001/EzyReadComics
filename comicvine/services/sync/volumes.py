...
from comicvine.api.client import fetch_volumes_by_ids
...
from comicvine.services.sync.results import VolumeBatchRefreshResult, VolumeListSaveResult
...
def get_or_create_volume_from_embedded_data(remote_volume, *, dry_run=False):
    ...
    existing_volume = ComicVineVolume.objects.filter(
    ...
    if existing_volume:
        ...
        if update_fields and not dry_run:
            with transaction.atomic():
                locked_volume = ComicVineVolume.objects.select_for_update().get(
    ...
    with transaction.atomic():
        volume = ComicVineVolume.objects.create(**volume_data)
...
def build_volume_list_data(remote_volume):
    ...
    if display_image_url:
        volume_data["display_image_source"] = (
            ComicVineVolume.IMAGE_SOURCE_COMICVINE_VOLUME
        )
    else:
        volume_data["display_image_source"] = ComicVineVolume.IMAGE_SOURCE_UNKNOWN
...
def save_volume_list_data(remote_volume, *, overwrite_existing=True, dry_run=False):
    ...
    local_volume = ComicVineVolume.objects.filter(comicvine_id=comicvine_id).first()
    ...
    if not local_volume:
        ...
        with transaction.atomic():
            local_volume = ComicVineVolume.objects.create(**create_data)
    ...
    with transaction.atomic():
        locked_volume = ComicVineVolume.objects.select_for_update().get(id=local_volume.id)
        update_fields = get_volume_list_update_fields(
...
def get_volumes_needing_list_data_refresh_queryset():
    ...
    return ComicVineVolume.objects.filter(needs_refresh_filter).order_by("id")
...
def refresh_missing_volume_list_data(
    *,
    ...
    if volume_ids:
        volumes_queryset = ComicVineVolume.objects.filter(
