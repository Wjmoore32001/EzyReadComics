...
USER_AGENT = "EzyReadComics manual_add_xmen_2024"
...
DEFAULT_RUN_DESCRIPTION = (
    "The current ongoing X-Men run from Marvel, beginning in 2024 as part of the "
    "From the Ashes era. Written by Jed MacKay with art by Ryan Stegman, this run "
    "follows Cyclops and his team after the end of the Krakoan Age."
)

COLLECTED_VOLUME_NUMBER = "1"
COLLECTED_VOLUME_FALLBACK_TITLE = "X-Men Vol. 1"
...
class Command(BaseCommand):
    help = (
        "Temporary catalog seeder for the current X-Men run. "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default=None,
            help=(
                "Optional plain Comic Vine volume ID for the current X-Men run. "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("Manual X-Men (2024) catalog seed"))
        ...
        if not remote_issues:
            raise CommandError("No X-Men issues were returned from Comic Vine.")
...
def resolve_current_run_volume_id(*, session, api_key, requested_volume_id):
    ...
    if not remote_matches:
        raise ComicVineAPIError(
            "Could not find one current X-Men volume on Comic Vine. "
    ...
    raise ComicVineAPIError(
        "Found multiple possible X-Men volumes. "
        "Run again with --comicvine-volume-id.\n"
...
def build_collected_volume_title(remote_issue_title):
    ...
    if cleaned_title.startswith(prefix):
        ...
        if subtitle:
            return f"X-Men Vol. 1: {subtitle}"

    return f"X-Men {cleaned_title}"
...
def build_collected_volume_description(collected_volume_data):
    ...
    return (
        "The first collected volume of the 2024 X-Men run by Jed MacKay and "
        "Ryan Stegman, collecting X-Men #1-6. "
