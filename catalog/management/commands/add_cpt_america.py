...
USER_AGENT = "EzyReadComics manual_add_captain_america_2025"
...
DEFAULT_RUN_DESCRIPTION = (
    "The current ongoing Captain America run from Marvel, beginning in 2025. "
    "Written by Chip Zdarsky with art by Valerio Schiti, this run launches a new era "
    "for Steve Rogers."
)

COLLECTED_VOLUME_NUMBER = "1"
COLLECTED_VOLUME_FALLBACK_TITLE = "Captain America Vol. 1"
COLLECTED_VOLUME_FIRST_ISSUE_NUMBER = "1"
COLLECTED_VOLUME_LAST_ISSUE_NUMBER = "5"
COLLECTED_VOLUME_ISSUE_NUMBERS = ["1", "2", "3", "4", "5"]


class Command(BaseCommand):
    help = (
        "Temporary catalog seeder for the current Captain America run. "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default=None,
            help=(
                "Optional plain Comic Vine volume ID for the current Captain America run. "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("Manual Captain America (2025) catalog seed"))
        ...
        if not remote_issues:
            raise CommandError("No Captain America issues were returned from Comic Vine.")
...
def resolve_current_run_volume_id(*, session, api_key, requested_volume_id):
    ...
    if not remote_matches:
        raise ComicVineAPIError(
            "Could not find one current Captain America volume on Comic Vine. "
    ...
    raise ComicVineAPIError(
        "Found multiple possible Captain America volumes. "
        "Run again with --comicvine-volume-id.\n"
...
def fetch_collected_volume_data(*, session, api_key):
    ...
    remote_issue_title = clean_text(collected_issue.get("title"))
    release_date = collected_issue.get("store_date")

    title = build_collected_volume_title(remote_issue_title)
...
def fetch_collected_volume_candidates(*, session, api_key):
    data = fetch_volumes_page(
        session,
        api_key,
        filter_value=f"name:{RUN_TITLE},start_year:2026",
        fields=VOLUME_LIST_FIELDS,
        offset=0,
        limit=100,
        sort="date_added:asc",
...
def build_collected_volume_title(remote_issue_title):
    cleaned_title = clean_text(remote_issue_title)

    if not cleaned_title:
        return COLLECTED_VOLUME_FALLBACK_TITLE

    prefix = "Vol. 1 - "

    if cleaned_title.startswith(prefix):
        subtitle = cleaned_title.removeprefix(prefix).strip()

        if subtitle:
            return f"Captain America Vol. 1: {subtitle}"

    return f"Captain America {cleaned_title}"
...
def build_collected_volume_description(collected_volume_data):
    ...
    return (
        "The first collected volume of the 2025 Captain America run by Chip Zdarsky "
        "and Valerio Schiti, collecting Captain America #1-5. "
        f"Collected-volume title/release source: {source_note}."
    )
